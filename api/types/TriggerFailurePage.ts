...
/**
 * A page of trigger execution failures as a `{data, nextCursor}` envelope. `nextCursor` is an opaque cursor — pass it back as `startFrom` to fetch the next page, or null when no more pages remain.
 */
export interface TriggerFailurePage {
    /** The items on this page, in the endpoint's natural order. Empty when there are no results. */
    data?: Vectros.TriggerFailureResponse[] | undefined;
